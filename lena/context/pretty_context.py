""":class:`PrettyContext` provides a better representation for context."""
import functools
import json
import warnings
...
def Context(*args, **kwargs):
    """.. deprecated:: 0.6 use :class:`PrettyContext`."""
    warnings.warn(
        "context.Context is renamed since Lena 0.6. "
        "Use context.PrettyContext. In:",
        DeprecationWarning, stacklevel=2
    )
    return PrettyContext(*args, **kwargs)


class PrettyContext(dict):

    """Easy-to-read dictionary.

    Use :class:`PrettyContext` to increase readability of large contexts
    from the flow. Example:

    >>> from lena.context import PrettyContext
    >>> d = {"1": 1, "2": {"3": 4}}
    >>> print(d)  # one-liner, hard to read for large contexts
    {'1': 1, '2': {'3': 4}}
    >>> c = PrettyContext(d)
    ...
    Now *Print()* and the output of the source *s* will have prettily
    formatted context.
    """
    ...
    def __init__(self, d=None, formatter=None):
        ...
        All public attributes of a :class:`PrettyContext`
        ...
        if d is None:
            d = {}
        super(PrettyContext, self).__init__(d)
    ...
    def __call__(self, value):
        """Convert *value*'s context to :class:`PrettyContext` on the fly.

        If the *value* is a *(data, context)* pair,
        convert its context part to :class:`PrettyContext`.
        If the *value* doesn't contain a context,
        it is created as an empty :class:`PrettyContext`.

        When a :class:`PrettyContext` is used as a sequence element,
        its initialization argument *d*
        has no effect on the produced values.
        """
        data, context = value
        # data, context = lena.flow.get_data_context(value)
        return (data, PrettyContext(context))
    ...
    def __getattr__(self, name):
        ...
        try:
            ...
            if isinstance(val, dict):
                ...
                if not isinstance(val, PrettyContext):
                    val = self[name] = PrettyContext(val)
                return val
            return val
    ...
    def _repr_nested(self, base_indent="", indent=" "*4, el_separ=",\n"):
        ...
        return base_indent + "PrettyContext()"
    ...
    def __setattr__(self, attr, value):
        if attr in ["_formatter"]:
            ...
            super(PrettyContext, self).__setattr__(attr, value)
